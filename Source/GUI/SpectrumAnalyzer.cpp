...
void SpectrumAnalyzer::drawCrossovers(juce::Graphics& g, juce::Rectangle<int> bounds)
{
    ...
    {
            return jmap(db, NEGATIVE_INFINITY, MAX_DECIBLES,
    ...
    g.fillRect(Rectangle<float>::leftTopRightBottom(lowMidX, zeroDb, midHighX, mapY(lowBandGr)));
    g.fillRect(Rectangle<float>::leftTopRightBottom(midHighX, zeroDb, right, mapY(lowBandGr)));
...
//        auto y = jmap(gDb, -24.f, 24.f, float(bottom), float(top));
        auto y = jmap(gDb, NEGATIVE_INFINITY, MAX_DECIBLES,
            (float)bottom, (float)top);
...
//                       -48.f, 0.f);
                       NEGATIVE_INFINITY, MAX_DECIBLES);
